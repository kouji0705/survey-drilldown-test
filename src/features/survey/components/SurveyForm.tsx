...
import {
  createSurveyResolver,
  getVisibleFieldIds,
  type SurveyDefinition,
  type SurveyFieldRepeat,
} from "@domain/survey";
import { ApiError, submitSurveyResponse } from "@infrastructure/api";
import { SurveyFieldRenderer } from "./SurveyFieldRenderer";
import { RepeatFieldSection } from "./RepeatFieldSection";

type Props = {
  /** POST 送信先のパスに使うアンケート ID */
  surveyId: string;
  /** GET で取得したアンケート定義（描画・分岐・Zod の元データ） */
  definition: SurveyDefinition;
};

/**
 * JSON 定義駆動のアンケート回答フォーム。
 *
 * - 表示: {@link getVisibleFieldIds} + `definition.fields` の map
 * - 検証: {@link createSurveyResolver}（Zod・送信時に visible 再計算）
 * - 送信: {@link submitSurveyResponse}
 */
